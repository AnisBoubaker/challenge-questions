...
la grille et afficher des murs ou? la grille contient un 1, et un espace ou? il n?y en a pas. Pour afficher un mur, on utilise le caractere ayant pour code
ASCII 219.
Si une solution est fournie (pile_solution != NULL), on doit identifier, sur
la grille afficheee, toutes les cases faisant partie de la solution (ex. par des X).
Parametres:
...
void lab_calculer_deplacements_possibles( int grille[][TAILLE_MAX_GRILLE],
                                          ...
                                          int pos_ligne,
                                          int pos_colonne,
                                          int solution_partielle[][2], int *nb_deplacements,
...
int lab_choisir_deplacement(
        int deplacements_possibles[][2], int nb_deplacements_possibles);
...
renvoie Faux. "position" est une sortie si elle se trouve en peripherie de la grille et que ce n'est pas un mur.
Parametres:
...
- debug : (entier) valeur booléeenne. Si sa valeur est Vraie, la fonction affichera la grille
au fur et a? mesure qu'on ajoute des cases a? la solution.
Retour: (Entier) Valeur vraie si une solution a e?te? trouve?e, ou fausse sinon.
Parame?tre modifie?s: pile_solution.
*/
