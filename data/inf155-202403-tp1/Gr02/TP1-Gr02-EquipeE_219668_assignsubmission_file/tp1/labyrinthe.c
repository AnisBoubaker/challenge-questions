...
//Tant que la pile_solution possède des valeurs ainsi qu'il ne s'agit pas d'une sortie
    while((!(lab_est_une_sortie(pile_solution[top_pile], grille, hauteur_grille, largeur_grille)) || ((pile_solution[top_pile][0] == entree[0]) && (pile_solution[top_pile][1] == entree[1]))) && (!pile_est_vide(pile_solution)))
    {
        ...
        //Chaque itération nous permets d'aller chercher le nombre de déplacement possible
        ...
        // On garde en réserve le pointeur dans nombre_deplacement_possible
        nombre_deplacement_possible = *nb_deplacements;

        //Le but est de choisir un déplacement parmi ceux qui sont a la portée du possible
        int le_deplacement = lab_choisir_deplacement(deplacement_possible, nombre_deplacement_possible);

        //Cela englobe les 4 possibilités de nombre de déplacement dans la mesure du possible
        if (nombre_deplacement_possible == 1) {
            //Nous voulons mettre dans le tableau empiler(pile_solution, déplacement choisi)
        ...
        {
            //Dans le cas ou il n'y a aucun déplacement possible, il faut dépiler (pile solution) et empiler (pile chemin alternatif)
            if (pile_est_vide(pile_chemain_alternatif) == 0)
            {
                ...
                //Nous enleveons la coordonée de top_pile dans le tableau chemin alternatif pour le mettre dans pile solution
        ...
        //Nous mettons pile solution dans pile_est_vide qui s'affectera dans pile_vide

        top_pile = pile_solution[0][1];

        if (debug) {
            //Nous voulons afficher le labyrinthe
