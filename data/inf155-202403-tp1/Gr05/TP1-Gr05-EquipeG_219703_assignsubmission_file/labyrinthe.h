...
Auteur: Francis Gignac, Rachelle Sanscartier, Maxim St-Gelais, Abraham Suarez-Santizo
...
Description: Affiche la grille du labyrinthe dans la console. La fonction doit parcourir
             ...
             en a pas. Pour afficher un mur, on utilise le caractère ayant pour code ASCII 219.

             Si une solution est fourni (pile_solution != NULL), on doit identifier, sur
             la grille affichée, toutes les cases faisant partie de la solution (ex. par des X).
Paramètres:
- grille : tableau à deux dimension d'entiers qui contient les informations
...
- pile_solution : (tableau 2D) pile de coordonées contenant la solution (partielle ou complète) du
                  labyrinthe.

Retour : Aucun.
Paramètre modifié : Aucun.
*/
...
*/
int lab_resoudre_profondeur(int grille[][TAILLE_MAX_GRILLE],int hauteur_grille,int largeur_grille,
                            int entree[2],int pile_solution[][2],int debug);
