...
la grille et afficher des murs o� la grille contient un 1, et un espace o� il n�y
en a pas. Pour afficher un mur, on utilise le caract�re ayant pour code
ASCII 219.
 Si une solution est fournie (pile_solution != NULL), on doit identifier, sur
 la grille affich�e, toutes les cases faisant partie de la solution (ex. par des X).
Param�tres:
...
Description: Choisit un d�placement parmi les d�placements possibles pass�s en param�tre.
 Dans cette version, le choix du d�placement se fait de fa�on al�atoire parmi les
 d�placement possibles. Cependant, si nous souhaitons faire �voluer notre programme
et choisir le prochain d�placement en utilisant une autre approche, il suffit de
changer cette fonction.
Param�tres:
- deplacements_possibles: tableau 2D contenant les coordonn�es des d�placements possibles. Les
 donn�es de ce tableau devraient �tre celles qui ont �t� obtenues par
...
Retour : (Entier) indice, dans le tableau deplacements_possibles qui a �t� choisi comme prochain
 d�placement.
Param�tres modifi�s : Aucun.
INF-155 Introduction � la programmation Automne 2023 / Page 8
*/
