...
Description:
    ...
    Charge un labyrinthe depuis le disque dur et le stocke dans "grille". Si une erreur
    se produit (fichier introuvable, fichier non-conforme), la fonction retourne faux. Sinon
    elle retourne vrai.

Parametres:

    - nom_fichier (chaine de caracteres): Le nom du fichier sur le disque à charger. ça doit être un fichier
    texte comprenant uniquement des * (pour les murs), des espaces (pour les couloirs et une lettre E pour
    ...
    - point_entree: (tableau de deux cases) Tableau dans lequel seront stockees les coordonnées du point d'entree.
...
Parametres modifiés : grille, point_entree, *hauteur_grille et *largeur_grille.
*/
...
Description:
    ...
    Affiche le menu principal et demande à l'utilisateur de saisir un choix.
...
Description:
    ...
    Demande à l'usager de saisir un nom de fichier correspondant au fichier contenant
