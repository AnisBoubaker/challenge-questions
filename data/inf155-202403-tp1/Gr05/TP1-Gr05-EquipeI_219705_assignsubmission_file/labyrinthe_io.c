...
int selection_menu(int grille_chargee)
{
    ...
    // Option 2 et 3 non accessible si la grille n'est pas chargée
    if (!grille_chargee)
    {
...
void menu_charger_labyrinthe(char *buf_nom_fichier, int buf_taille)
{
    ...
    // Saisir le nom du fichier dans buf_nom_fichier
    ...
    // Imprimer le nom du fichier demandé
    printf("%s \n", buf_nom_fichier);

    // Affichage d'un message d'erreur lorsque le nom du fichier est trop long
